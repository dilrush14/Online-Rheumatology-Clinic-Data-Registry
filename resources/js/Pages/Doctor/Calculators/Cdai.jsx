...
export default function Cdai() {
  ...
  const save = () => {
    const payload = {
      ...
      category,
      // Optional if your table stores joint ids like DAS28:
      tender_joints: tender,
      swollen_joints: swollen,
    };

    // IMPORTANT: use router.post(url, payload, options)
    ...
    <AppShell>
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold">CDAI · {patient?.name}</h1>
          <Link
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          {/* Spacer column on md screens so layout mirrors DAS28 nicely */}
          <div className="rounded-2xl border p-4 bg-white shadow-sm">
            <div className="text-sm text-slate-600">Formula</div>
            <div className="mt-2 text-sm">
              CDAI = TJC28 + SJC28 + Patient Global (0–10) + Physician Global (0–10)
            </div>
          ...
          <div className={`rounded-2xl border p-4 bg-white shadow-sm ring-2 ${meta.panel}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <span
                className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-sm font-medium ${meta.badge}`}
              >
                {category}
              </span>
            </div>
